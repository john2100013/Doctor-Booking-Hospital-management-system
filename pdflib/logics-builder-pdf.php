...
class LB_PDF extends FPDF {
    ...
    public function Header() {
        if ($this->pageHeaderRepeat || (!$this->pageHeaderRepeat && !$this->pageHeaderAdded)) {
            // Logo
            $imagePath = __DIR__ . './dist/img/logo.png';

if (file_exists($imagePath)) {
    $this->Image($imagePath, $this->GetX(), $this->GetY(), 30);
} else {
    // Output a placeholder or an error message
    $this->Cell(30, 10, 'Image not found', 1, 0, 'C');
}
